...
tour_data <- tour_data %>%
  mutate(dist_crow_km = get_geo_distance(start_long, start_lat, end_long, end_lat) / 1000) 
...
dist_error <- tour_data %>%
  filter(dist_crow_km - distance_km > 40)
...
Manual corrections to stages which stil have dist_crow_km > distance_km + 40km
```{r stage_manual}
tour_data <- select(tour_data, -dist_crow_km)
...
Recalc dist_crow_km.  Two cases, decided these need to know adjustment (as noted above).
```{r check_dist_4}
# Calculate distance between start and end of stage. 
tour_data <- tour_data %>%
  mutate(dist_crow_km = get_geo_distance(start_long, start_lat, end_long, end_lat) / 1000) 
...
```{r get_elevation}
# Functions to map (get elevation from geonames).  Use one of two surveys, topo30 or srtm3.
get_elev_topo30 <- function(lat, long){
  geonames::GNgtopo30(lat, long)$gtopo30
}

get_elev_srtm3 <- function(lat, long){
  geonames::GNsrtm3(lat, long)$srtm3
}

# Construct unique list of GPS coordinates (to reduce number of queries to geonames api)
gps_summary_start <- tour_data %>%
  select(starts_with("start"))

gps_summary_start <- gps_summary_start %>%
  rename_all(.funs = funs(sub("+.+_", "", names(gps_summary_start)))) 

gps_summary_end <- tour_data %>%
  select(starts_with("end"))

gps_summary_end <- gps_summary_end %>%
  rename_all(.funs = funs(sub("+.+_", "", names(gps_summary_end)))) 

gps_summary <- gps_summary_start %>%
  dplyr::bind_rows(gps_summary_end) %>%
  distinct()

# Map get_elev over each location.  Split into 3 as can be temperamental connecting to geonames api (could be my internet connection)
gps_summary_1 <- gps_summary %>%
  slice(1:300) %>%
  mutate(elev = map2_dbl(lat, long, get_elev_topo30))

gps_summary_2 <- gps_summary %>%
  slice(301:550) %>%
  mutate(elev = map2_dbl(lat, long, get_elev_topo30))

gps_summary_3 <- gps_summary %>%
  slice(551:791) %>%
  mutate(elev = map2_dbl(lat, long, get_elev_topo30))

# Combine gps_summary together
gps_summary <- dplyr::bind_rows(gps_summary_1, gps_summary_2, gps_summary_3)

# Get elevation from strm
elev_error <- filter(gps_summary, elev < -20 | elev > 3000) %>%
  select(-elev) %>%
  mutate(elev = map2_dbl(lat, long, get_elev_srtm3))

# Add corrected elevations to gps_summary
gps_summary <- gps_summary %>%
  anti_join(elev_error, by = c("place", "country", "lat", "long")) %>%
  bind_rows(elev_error)

# Check for any errors
gps_summary %>%
  filter(elev < -20 | elev > 3000)

# One error, Passge du Gois. Probably an error because this is an underwater causeway only passable at sometimes of the day. Give it an elevation of -1 as it is frequently underwater.
gps_summary$elev[gps_summary$place == "Passage du Gois"] <- -1

top_n(gps_summary, 10, elev) %>% arrange(-elev)
top_n(gps_summary, 10, -elev) %>% arrange(elev)
```

Add elevation to tour_data
```{r add_elevation}
# Add start location elevations
tour_data <- tour_data %>%
  rename_all(.funs = funs(sub("start_", "", names(tour_data)))) %>%
  left_join(gps_summary, by = c("place", "country", "lat", "long", "gnid")) %>%
  rename(
    start_place = "place",
...
# Add end location elevations
tour_data <- tour_data %>%
  rename_all(.funs = funs(sub("end_", "", names(tour_data)))) %>%
  left_join(gps_summary, by = c("place", "country", "lat", "long", "gnid")) %>%
  rename(
    end_place = "place",
    end_country = "country",
...
tour_data <- tour_data %>%
  rename(distance_crow_km = "dist_end")
...
# Check no stages with same date apart from substages.  Returned 11 stage numbers on same date. Checked against Tour website.  Changed if neccessary.
dupl_dates <- tour_data %>%
  filter(sub_stage != "a", sub_stage != "b", sub_stage != "c") %>%
  group_by(date) %>%
  summarise(n_date = n()) %>%
  filter(n_date > 1)

tour_data %>%
  semi_join(dupl_dates)

# 1970_P
row <- tour_data$stage_id == "1970_P"
tour_data$date[row] <- make_date(1970L, 6L, 26L)

# 1975_5
row <- tour_data$stage_id == "1975_5"
tour_data$date[row] <- make_date(1975L, 7L, 1L)

# 1984 correct, no change

# 1986 correct, no change

# 1987 correct, no change (both (1,2) and (4,5))

# 1988 correct, no change (both (1,2) and (16,17))

# 1989 correct, no change

# 1990 correct, no change

# 1991 correct, no change
```
