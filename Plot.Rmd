...
```{r plot_stages}
# Create day variable which shows the day of the Tour with reference to the start date of the first stage for that year.
...
# Create variable tour_day to be day of tour.
tour_data <- tour_data %>%
  arrange(date) %>%
  group_by(year) %>%
...
tour_data %>%
  ...
  geom_tile () +
  scale_fill_manual(values=c("#008B00", "#FFD700","#0000EE", "#CD0000", "#000000","#9400D3", "#00B2EE")) + 
